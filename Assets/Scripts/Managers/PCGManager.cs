    ...
    public class PCGManager : MonoBehaviour
    {
        ...
        void SpawnPlatform(Vector2 pos, int length)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                GameObject newTile = Instantiate(LevelElements.TileTypes[tileIndex].Prefab, tilePos, Quaternion.identity);
                _tiles.Add(newTile);
                if (i == length / 2)
                {
                    TileType currentTile = LevelElements.TileTypes[tileIndex];
                    if (currentTile is TimedTile || currentTile is FallingTile)
                    {
                        continue;
                    }
                    else
        ...
        void SpawnObstacles()
        {
            ...
            for (int i = LevelElements.InitialTiles; i < _tiles.Count; i++)
            {
                ...
                else
                {
                    ...
                    else if (currentObstacle is FallingSpike)
                    {
                        Transform fallingSpikeTransform = newObstacle.transform;
                        fallingSpikeTransform.position = new Vector2(fallingSpikeTransform.position.x, fallingSpikeTransform.position.y + 9.5f);
                    }
                }
            }
        }
        ...
        {
            GameObject levelEnd = Instantiate(LevelEndPrefab, _nextPosition, Quaternion.identity);
            _tiles.Add(levelEnd);
        }
